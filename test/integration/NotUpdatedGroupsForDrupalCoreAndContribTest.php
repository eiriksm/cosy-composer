...
';
    ...
    public function testWhyNotUpdatesGroupedContribAndCore()
    {
        $this->runtestExpectedOutput();
        $output = $this->cosy->getOutput();
        // All of these should be attempted a why not on.
        ...
        foreach ($packages_and_versions as $package => $version) {
            $found = false;
            foreach ($output as $message) {
                ...
                if (strpos($context['command'], "composer why-not $package $version") === 0) {
                    $found = true;
                }
            }
            self::assertTrue($found, 'Did not find why not for ' . $package);
        }
    }
